...
router.get('/byCategoryId', (req, res) => {
    utils.getConn().query({
        ...
        let maxPages = Math.ceil(table[0].cnt / 5);
        utils.getConn().query({
            sql: 'select post_id, category_name, post_date, post_title, post_content as post_shot from post inner join category where category.category_id = post.post_category_id and post.post_category_id = ? order by post_date desc limit ?',
